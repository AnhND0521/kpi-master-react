...
const TemplateDialog = ({ open, setOpen, setCurrentKpis }) => {
  ...
  return (
    <Dialog open={open} handler={() => setOpen(!open)}>
      ...
      <DialogBody className="text-black px-16 py-10 pt-0 h-[40rem] overflow-y-scroll">
        <div className="flex flex-col items-start gap-6">
          <Typography className="px-3 text-sm">
            Áp dụng ngay các mẫu KPI được sử dụng nhiều trong hệ thống!
          </Typography>
          <Carousel>
            {templates.map((t, i) => (
              <div className="flex items-center justify-center gap-4 bg-[#d6d0f7] py-10">
                {t.map((k, j) => (
                  <Button
                    className="bg-white w-32 py-7 text-black"
                    onClick={() => setIndex([i, j])}
                  >
                    {k.name}
                  </Button>
                ))}
          ...
          <Typography variant="h5" className="text-lg font-medium">
            Mẫu: {templates[index[0]][index[1]].name}
          </Typography>
      ...
      <DialogFooter>
        <Button className="w-full bg-purple" onClick={handleSubmit}>
          Thêm
        </Button>
      </DialogFooter>
    </Dialog>
